...
class LLMExtractor:
    @staticmethod
    async def extract(reviewed_text: str) -> List[CaptureCandidate]:
...
CRITICAL RULES:
1. If a candidate is an EXACT duplicate of a context node (same meaning, 
   same patient, same drug), SKIP it.
2. If a candidate REINFORCES or ADDS NEW DETAIL to a context node 
   (new refusal count, new symptom, dose change), EXTRACT IT — 
   it will be handled as an UPDATE during conflict detection.
3. If a candidate is genuinely new, EXTRACT IT.
